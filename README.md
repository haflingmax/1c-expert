...
- Не обновляет статистику - на современных серверах бесполезна и бессмысленна.
...
- Удаляет старый индекс, создает новый. Блокирует весь индекс. Автоматически обновляет статистику по обслуживаемому индексу. Но лучше просто обновите статистику.
```sql
ALTER INDEX index_name ON table_name REBUILD;
```
#### 1.3. Мониторинг MS SQL Server
...
#### 2.2. Windows 
##### 2.2.1. Сбор информации по загруженности оборудования <Все рабочие сервера кластера 1С:Предприятия>:
- Открыть Performance Monitor: `perfmon`
- Добавить счетчики:
